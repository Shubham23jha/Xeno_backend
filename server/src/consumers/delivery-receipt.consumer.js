...
const handleMessage = async (message) => {
  try {
    ...
    if (message.type === "UPDATE_COMMUNICATION_LOG") {
      ...
      console.log(
        `[x] Updating CommunicationLog with id: ${id}, status: ${status}`
      );
    ...
    if (message.type === "UPDATE_COMMUNICATION_LOG") {
      console.error(
        `[x] Error handling UPDATE_COMMUNICATION_LOG message: ${err}`
      );
    } else {
...
const startBatchProcessor = () => {
  setInterval(async () => {
    if (messageQueue.length >= BATCH_SIZE && !isProcessing) {
      ...
      isProcessing = true;
      console.log(
        `[x] Processing messages in queue. Queue length: ${messageQueue.length}`
      );
      ...
      if (!wasProcessing) {
        console.log(
          `[x] Batch interval reached. Currently processing a batch. New messages will be processed in the next interval.`
        );
        wasProcessing = true;
      }
    } else {
      if (!wasQueueEmpty) {
        console.log(
          `[x] Batch interval reached. Message queue is empty. No messages to process.`
        );
        wasQueueEmpty = true;
      }
    }
...
const startConsumer = async () => {
  try {
    ...
    consumeMessages((message) => {
      console.log(
        `[x] Message received for queueing: ${JSON.stringify(message)}`
      );
      ...
      if (messageQueue.length >= BATCH_SIZE && !isProcessing) {
        isProcessing = true;
        console.log(
          `[x] Batch size reached. Processing batch of ${BATCH_SIZE} messages`
        );
        const messagesToProcess = messageQueue.splice(0, BATCH_SIZE);
        ...
        console.log(
          `[x] Message queued. Current queue length: ${messageQueue.length}`
        );
      }
    });
    startBatchProcessor();
