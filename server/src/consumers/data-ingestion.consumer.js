  ...
  try {
    ...
    switch (message.type) {
      case "CREATE_CUSTOMER":
        console.log(
          `[x] Handling CREATE_CUSTOMER with payload: ${JSON.stringify(
            message.payload
          )}`
        );
        try {
      ...
      case "CREATE_ORDER":
        console.log(
          `[x] Handling CREATE_ORDER with payload: ${JSON.stringify(
            message.payload
          )}`
        );
        try {
...
const startConsumer = async () => {
  try {
    ...
    consumeMessages(handleMessage);  // Make sure this is imported correctly
  } catch (err) {
